...
const CONFIG = {
    ...
    REFRESH_INTERVAL: 60000, // 60 seconds - matches EcoWitt push interval
...
async function fetchAqiData() {
  try {
    ...
    const data = await res.json();
    if (data.error) return;
    const fmt = v => v !== null ? `${v} �g/m�` : '--';
    document.getElementById('pm25').innerHTML = `${data.pm25} <span style="font-size:0.5em">�g/m�</span>`;
    document.getElementById('aqi-status').textContent = getAqiStatus(data.pm25);
    document.getElementById('pm25-now').textContent = fmt(data.pm25);
    document.getElementById('pm25-24h').textContent = fmt(data.pm25_24h);
  } catch (err) {
